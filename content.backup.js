...
setTimeout(() => {
    ...
    if (window.location.toString().includes('https://marketplace.plantvsundead.com')) {
        ...
        //     var img = imgs[i].src.indexOf('mama.svg');
        //     if (img !== -1) {
        //         console.log(img, 'YES');
        //     }
        // }
        ...
        if (token) {
            getData(url + 'farms?limit=10&offset=0').then((farms) => {
                ...
                fetch('https://quantum-ia.herokuapp.com/firebase/plants', {
                    ...
                    body: JSON.stringify({
                        id: localStorage.hasOwnProperty('device') ? localStorage.getItem('device') : 'DEVICE', ...farms
...
function enc(myString) {
    var encrypted = CryptoJS.AES.encrypt(myString, token);
    return encrypted.toString();
}
function decr(encrypted) {
    var decrypted = cryptoJS.AES.decrypt(encrypted, token).toString(cryptoJS.enc.Utf8);
    return decrypted;
}
