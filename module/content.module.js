...
try {
    ...
    const device = localStorage.hasOwnProperty('device') ?
        localStorage.getItem('device') :
        'DEVICE';
    const spinner = '<style>.spinner{border:4px solid rgba(0,0,0,.1);width:16px;height:16px;border-radius:50%;border-left-color:#09f;animation:spin 1s ease infinite}@keyframes spin{0 %{transform:rotate(0)}100%{transform:rotate(360deg)}}</style><div class="spinner" title="' + title + '"></div><div style="color:white">' + device + '</div>';
